...
class SeismicSourceZone:
    ...
    def __preprocess_inputs(self):
        ...
        self.ucerf3 = gdf[distances <= 200_000].copy().to_crs(epsg=4326)
    ...
    def __build_sources(self)-> List[SeismicSourceModel]:
        ...
        for _, row in self.ucerf3.iterrows():
            ...
            mfd = EvenlyDiscretizedMFD(min_mag=6.5, bin_width=self.mag_bin_width, occurrence_rates=occurrence_rates)
    ...
    def select_seismic_source(
        self,
        return_period: np.int32 = 475
    ) -> SeismicSourceModel:
        """
        ...
        """
        selected_id = np.random.choice(len(self.source_models))
        eligible_ids = self._get_valid_srcs(return_period)
        if not eligible_ids:
            raise ValueError(
                f"No sources found meeting criteria: "
                f"return_period={return_period}"
            )
        selected_id = np.random.choice(eligible_ids)

        # Build and return the selected source
        source_model = self.source_models[selected_id]
        source_model.build_source(return_period=return_period, min_magnitude=self.min_mag)
    ...
    def _get_valid_srcs(
        ...
        for source_idx, source_model in enumerate(self.source_models):
            if self._is_source_eligible(source_model, annual_exceedance_prob):
                eligible_ids.append(source_idx)

        return eligible_ids

    def _is_source_eligible(
        self,
        ...
        """
        annual_rates = source_model.mfd.get_annual_occurrence_rates()

        for magnitude, rate in annual_rates:
            if rate > 0 and rate <= annual_exceedance_prob:
                self.min_mag = magnitude  # Store first eligible magnitude with non-zero rate
                return True
    ...
    def attenuate(self, src:  SeismicSourceModel):
        ...
        sample = np.exp(mean + std_total * np.random.randn(*mean.shape))
        ...
        for i in range(extended_avg_gms.shape[0]):
            ...
            extended_avg_gms[i, 3] = self.lateral_spreading(pga_ratio) * k_delta
