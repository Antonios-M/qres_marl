...
def plot_recovery_timesteps(inference="all", threshold_ratio=0.70, n=10,
                            bins=30, figsize=(12, 10), save_path=None):
    """
    Plots distribution of the number of timesteps required to recover a specified
    proportion of the initial loss (e.g., 50%) for each policy.

    Parameters:
        inference (str or list): Policy name(s) or "all"
        threshold_ratio (float): The target recovery proportion (e.g., 0.5 for 50%)
        n (int): Number of rollouts per policy
    ...
    def recovery_timestep(_l, threshold_ratio=0.50):
        l = [abs(x) for x in _l]
        if not l:
            return None
        target = l[0] - (l[0] * threshold_ratio)
        # print(f"l: {l}")
        count = 0
    ...
    for name in policy_list:
        ...
        steps = [recovery_timestep(l, threshold_ratio) for l in losses]
        steps = [s for s in steps if s is not None]

        if steps:
            all_timesteps[name] = steps
        else:
            print(f"No valid data for '{name}'")

    if not all_timesteps:
        print("No valid timestep data across policies.")
        return

    active_policies = list(all_timesteps.keys())
    ...
    all_values = [v for steps in all_timesteps.values() for v in steps]
    min_val, max_val = min(all_values), max(all_values)
    bins_range = np.linspace(min_val, max_val, bins + 1)

    for i, name in enumerate(active_policies):
        ax = axes[i]
        steps = all_timesteps[name]
        mean_val, std_val = np.mean(steps), np.std(steps)

        sns.histplot(steps, bins=bins_range, ax=ax, stat="density", kde=True,
        ...
        ax.set_xlabel("Timesteps to Recovery")
        ax.set_ylabel("Density")
        ax.set_title(policy_title_map.get(name, name))

    kde_ax = axes[num_policies]
    for name, steps in all_timesteps.items():
        sns.kdeplot(steps, label=policy_title_map.get(name, name), ax=kde_ax,
                    color=policy_colors[name], fill=True, alpha=0.3)

    kde_ax.set_xlim(min_val, max_val)
    kde_ax.set_xlabel(f"Timesteps to {int(threshold_ratio * 100)}% Recovery / {env.time_step_duration} days")
    kde_ax.set_ylabel("Density")
    kde_ax.set_title(f"KDE Overlay: Timesteps to {int(threshold_ratio * 100)}% Recovery")
    kde_ax.legend()
    ...
    fig.suptitle(
        f"$\\bf{{Timesteps\\ to\\ {int(threshold_ratio * 100)}\\%\\ Recovery}}$ "
        f"Distribution per Policy, over {n} Rollouts",
...
# plot_recovery_timesteps(inference=["random", "importance_based", "DCMAC"], n=1000, figsize=(12, 8))
