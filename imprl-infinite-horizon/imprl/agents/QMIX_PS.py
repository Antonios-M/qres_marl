    ...
    def __init__(self, n_agents, state_dim, mixer_config):
        ...
        if mixer_config["hypernet_hidden_units"] in [0, None]:
            self.hypernet_w1 = nn.Linear(
                self.state_dim, self.mixer_embed_dim * self.n_agents
            )
            ...
            self.hypernet_w1 = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(
                    self.hypernet_hidden_units, self.mixer_embed_dim * self.n_agents
                ),
            )
...
class QMIXParameterSharing(ValueAgent):
    ...
    def process_experience(self, *args):
        ...
        system_obs = self.env.system_percept(obs)
        ma_idx_obs = self.env.multiagent_idx_percept(obs)
        system_next_obs = self.env.system_percept(next_obs)
        ma_idx_next_obs = self.env.multiagent_idx_percept(next_obs)
        state = self.env.system_percept(state)
        next_state = self.env.system_percept(next_state)
    ...
    def compute_current_values(self, t_ma_obs, t_states, t_beliefs, t_actions):
        ...
        # shape: (batch_size, n_agents, n_comp_actions)
    ...
    def _preprocess_inputs(
        ...
        t_ma_next_beliefs = torch.tensor(np.array(ma_idx_next_beliefs)).to(self.device)
        ...
        t_states = torch.tensor(np.array(states)).to(self.device)
        t_next_states = torch.tensor(np.array(next_states)).to(self.device)
