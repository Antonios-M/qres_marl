...
class MultiAgentWrapper:
    ...
    def __init__(self, env) -> None:
        ...
        self.local_observation_space = Tuple(
            tuple([Box(0, 1, shape=(1,), dtype=self.dtype)] * self.core.n_agents)
        )
        ...
        self.action_space = Tuple(
            tuple(
                [Discrete(k) for k in self.core.per_agent_actions],
            )
        )
    ...
    def multiagent_percept(self, percept):
        ...
        if self.core.time_perception:
            shared_percept = spaces.utils.flatten(self.shared_observation_space, percept[0])
            shared_percept = np.broadcast_to(shared_percept, (1, self.core.n_agents))

            local_percept = spaces.utils.flatten(self.local_observation_space, percept[1].T)
            ...
            local_percept = spaces.utils.flatten(self.local_observation_space, percept.T)
            local_percept = local_percept.reshape(-1, self.core.n_agents, order="F")
