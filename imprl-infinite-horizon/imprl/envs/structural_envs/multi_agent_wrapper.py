...
class MultiAgentWrapper:
    ...
    def __init__(self, env) -> None:
        ...
        if self.core.percept_type in ["obs", "Obs", "state", "State"]:
            ...
            self.local_observation_space = tuple(
                [Box(0, 1, shape=(self.core.n_damage_states,), dtype=self.dtype)]
                * self.core.n_components
            )
        self.local_observation_space = Tuple(self.local_observation_space)
        ...
        self.action_space = Tuple(
            tuple(
                [Discrete(self.core.n_comp_actions)] * self.core.n_components,
            )
        )
    ...
    def multiagent_percept(self, percept):
        ...
        if self.core.time_perception:
            ...
            local_percept = local_percept.reshape(-1, self.core.n_components, order="F")
            shared_percept = spaces.utils.flatten(
                self.shared_observation_space, percept[0]
            )
            shared_percept = np.broadcast_to(
                shared_percept, (1, self.core.n_components)
            )
